...
- If any square has $\mathrm{num}(\text{surrounding mines}) = \mathrm{num}(\text{surrounding flags})$, all other surrounding squares are safe
- If any square has $\mathrm{num}(\text{surrounding mines}) = \mathrm{num}(\text{surrounding uncovered squares})$, they are all mines
